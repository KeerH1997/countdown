...
const QUOTES = [
  ...
  "你不是在堆砌文字，你是在为国际秩序的演进贡献一份确定的逻辑。",
  "此时的痛苦，正是你的思维在复杂法理中进行“精细化主权裁决”的过程。",
  "既然选了最难的国际法，就别在终点线前怀疑自己对正义和规则的直觉。",
  "论文的每一个注脚，都是你与前沿学者跨越时空的“学术缔约”。",
  "把那些晦涩的案例和条约拆解开，你会发现答案就藏在你的论证逻辑里。",
  "不要怕论点有瑕疵，国际法本身就在不断修正中走向成熟，你的论文亦然。",
  "此时敲下的每一个字，都在增强你作为法律学者面对复杂世界时的解释力。",
  "最后十天，实用主义高于一切：先让全文贯通，再谈辞藻华丽。",
  "像审查条约草案一样审查你的目录，只要骨架稳了，论文就倒不了。",
  "卡在某个争议判例上时，先跳过去，别让局部的“领土争端”拖慢了全局的进度。",
  "现在的每一分钟，都是在为你未来的答辩积累“不被问倒”的资本。",
  "哪怕只写出几行有力的论证，也比反复阅读那些已经滚瓜烂熟的文献更有用。",
  "论文不是完美的工艺品，它是你此刻学术生命最真实的“快照”。",
  "此时的键盘声，是你对多年枯燥研究最响亮的回应。",
  "最后的冲刺是“习惯法”的养成，只要每天坐下来，写作就会变成一种必然。",
  "你不需要在这十天里解决全球治理的所有问题，你只需要讲清楚你的那一部分。",
  "别把论文当成终身成就，它只是你进入学术共同体的一张通行证。",
  "如果觉得最难熬的时候，其实正是论文即将“合龙”的前夜。",
  "写下去，为了那个即将称命你为“Doctor”的时刻。"
];
...
function updateDateAndCountdown() {
  ...
  countdownLabel.textContent = `目标日已过去 ${Math.abs(dayDelta)} 天。秹续保持写作惯性，把收尾做扎实。`;
  queueFitCalendar();
}
...
async function fetchWeather() {
  ...
  try {
    ...
    if (hasWeatherData) {
      ...
      setWeatherStatus("天气梂时没连上。点击右上角“刷新”再试一次。", true);
